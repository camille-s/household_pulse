...
loss <- readRDS("data/clean_data/employ.RDS") %>%
  ...
  mutate(exp_loss =round( 100*Yes / (No+Yes), 1), 
...
food_plot <- ggplot(food, aes(x = race, fill = area, y = foodscarcity,
                              label = paste0(round(foodscarcity,1), "%"))) + 
...
```{r micro_import}
micro <- vroom("data/microdata/pulse2020_puf_01.csv") %>% 
  bind_rows(vroom("data/microdata/pulse2020_puf_02.csv"), 
            vroom("data/microdata/pulse2020_puf_03.csv")) %>% 
...
hous <- bind_rows(hous_us, hous_ct, hous_nyc) %>% 
...
hous_plot <- hous %>% ggplot(aes(x = race, y = noconf, fill = area)) + 
	    geom_bar(stat = "identity", position = position_dodge()) + 
	    geom_text(aes(label = paste0(round(noconf, 1), "%"),
	                  y = noconf + 1), 
	              position = position_dodge(width = .9), size = 3.5) + 
	    annotation_custom(logo, xmin = 4.3, xmax = 4.8, ymin = -6, ymax = -10) +
...
loss <- bind_rows(loss_us, loss_ct, loss_nyc) %>% 
  filter(str_detect(RRACE, "White|Asian|(Hispanic,)|Black")) %>% 
...
loss_plot <- loss %>% ggplot(aes(x = race, y = loss, fill = area)) + 
	    geom_bar(stat = "identity", position = position_dodge()) + 
	    geom_text(aes(label = paste0(round(loss, 1), "%"),
	                  y = loss + 1.2), 
	              position = position_dodge(width = .9), size = 3.5) + 
	    annotation_custom(logo, xmin = 4.1, xmax = 4.8, ymin = -9, ymax = -15) +
	    labs(title= "Pandemic-related income loss (change title)",
	         subtitle = "Data from Weeks 1 to 3 of the US Household Pulse Survey", 
...
food <- bind_rows(food_us, food_ct, food_nyc) %>% 
  filter(str_detect(RRACE, "White|Asian|(Hispanic,)|Black")) %>% 
  mutate(race = str_wrap(RRACE,15)) %>% 
...
food_plot <- food %>% ggplot(aes(x = race, y = food, fill = area)) + 
	    geom_bar(stat = "identity", position = position_dodge()) + 
	    geom_text(aes(label = paste0(round(food, 1), "%"),
	                  y = food + .5), 
	              position = position_dodge(width = .9), size = 3.5) + 
	    annotation_custom(logo, xmin = 4.1, xmax = 4.6, ymin = -3, ymax = -5) +
...
unemp_plot <- unemp %>% ggplot(aes(x = race, y = unemp, fill = area)) + 
	    geom_bar(stat = "identity", position = position_dodge()) + 
	    geom_text(aes(label = paste0(round(unemp, 1), "%"),
	                  y = unemp + 1.2), 
	              position = position_dodge(width = .9), size = 3.5) + 
	    annotation_custom(logo, xmin = 4.1, xmax = 4.8, ymin = -9, ymax = -15) +
